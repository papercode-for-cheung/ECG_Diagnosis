...
#        sample['data'] = np.average(sample['data'], axis=2)
        sample['data'] = utils.remove_outlier(sample['data'])
        sample['data'] = utils.remove_noise(sample['data'])
...
class CNN_Linear(nn.Module):
    ...
    def forward(self, x):
        ...
        fbout = x[:, :, :256] + x[:, :, 256:]  # seq_len batch_size hidden_units
        fbhn = (h_n[-2, :, :] + h_n[-1, :, :]).unsqueeze(0)
        #        #print (fbhn.shape, fbout.shape)
        attn_out = attnetwork(fbout, fbhn)
