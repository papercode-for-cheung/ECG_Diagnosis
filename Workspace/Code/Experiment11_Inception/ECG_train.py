...
import drawing
#%% Input Arguments
parser = argparse.ArgumentParser(description='Experiment11(inception): Train the model for diagnosing the heart disease by the ECG.')
...
if __name__ == '__main__':
    ...
    #%% ########## Create model ##########
    inception = models.Inception3().cuda() if Args.cuda else models.Inception3()
    inception.aux_logits = False
    # optimizer
    optimizer = torch.optim.Adam(inception.parameters(), lr=Args.learn_rate)
    # loss function
    ...
    for epoch in range(Args.epoch):
        ...
        F1.append(f1_test)
        if Args.show_plot:
            drawing.draw_result([Accuracy, F1], fig, ['INCEPTION Accuracy', 'INCEPTION F1'], True)
        # save
