...
import drawing
#%% Input Arguments
parser = argparse.ArgumentParser(description='Experiment10(VGG16): Train the model for diagnosing the heart disease by the ECG.')
...
if __name__ == '__main__':
    ...
    #%% ########## Create model ##########
    vgg16 = models.vgg16_bn().cuda() if Args.cuda else models.vgg16_bn()
    # optimizer
    optimizer = torch.optim.Adam(vgg16.parameters(), lr=Args.learn_rate)
    # loss function
    ...
    for epoch in range(Args.epoch):
        ...
        F1.append(f1_test)
        if Args.show_plot:
            drawing.draw_result([Accuracy, F1], fig, ['VGG16 Accuracy', 'VGG16 F1'], True)
        # save
